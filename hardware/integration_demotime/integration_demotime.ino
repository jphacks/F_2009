...
void loop() {
  ...
  while(true){
    ...
    while(true){
      ...
      Serial.println();
      if((distance > 30) && (diff_time > 3) && !ongoingFlag){  // true
        // break
        break;
      }
      ...
      current_time = millis();
      diff_time = (current_time - start_time) / 1000;
      if((diff_time > 70) && !ongoingFlag && !bathalertFlag){  // true
        // Bottomloop2
        /*
        // ongoing
        ...
        StaticJsonDocument<capacity> json_request;
        Serial.println((const char*)global_hash);
        // JSON構造化
        json_request["device_id"] = device_id;
        json_request["tag_id"] = global_tag_id;
        delay(200);
        ...
        */
        ...
        // JSONパース
        deserializeJson(doc_after_alert, json_after_alert);
        // Output
        ...
        current_time = millis();
        subdiff_time = (current_time - substart_time) / 1000;
        if(((subdiff_time) > 20) || ongoingFlag){  // true
          ongoingFlag = false;
          // ongoing
          displayValues("Reporting to", "Server...");
          // JSONオブジェクト作成
